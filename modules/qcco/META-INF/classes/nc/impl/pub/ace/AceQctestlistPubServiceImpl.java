...
import nc.bs.qcco.qctestlist.ace.bp.AceQctestlistInsertBP;
import nc.bs.qcco.qctestlist.ace.bp.AceQctestlistUpdateBP;
import nc.bs.qcco.qctestlist.ace.bp.AceQctestlistDeleteBP;
...
import nc.vo.qcco.qctestlist.TestlistcompBVO;
import nc.vo.qcco.qctestlist.TestlistentryEVO;

public abstract class AceQctestlistPubServiceImpl {
	...
	// 新增
	public AggTestlistHVO[] pubinsertBills(AggTestlistHVO[] vos)
			throws BusinessException {
		try {
			// 数据库中数据和前台传递过来的差异VO合并后的结果
			BillTransferTool<AggTestlistHVO> transferTool = new BillTransferTool<AggTestlistHVO>(
					vos);
			AggTestlistHVO[] mergedVO = transferTool.getClientFullInfoBill();

			// 调用BP
			AceQctestlistInsertBP action = new AceQctestlistInsertBP();
			AggTestlistHVO[] retvos = action.insert(mergedVO);
			// 构造返回数据
	...
	// 删除
	public void pubdeleteBills(AggTestlistHVO[] vos) throws BusinessException {
		try {
			// 加锁 比较ts
			BillTransferTool<AggTestlistHVO> transferTool = new BillTransferTool<AggTestlistHVO>(
					vos);
			AggTestlistHVO[] fullBills = transferTool.getClientFullInfoBill();
			...
			for (String tableCode : tableCodes) {
				...
				for (SuperVO childVO : originChildrens) {
					// 将当前页签下的当前子的所有孙都查询出来了,并赋值给originBills中的孙。
						String originChildPK = ((TestlistcompBVO) childVO)
								.getPrimaryKey();
						Collection originGVOs = query.queryBillOfVOByCond(
								TestlistentryEVO.class, "pk_testlistcomp = '"
										+ originChildPK + "' and dr=0", false);
						TestlistentryEVO[] originGrandvos = (TestlistentryEVO[]) originGVOs
								.toArray(new TestlistentryEVO[originGVOs.size()]);
						((TestlistcompBVO) childVO).setPk_testlistentry(originGrandvos);
...
//						this.deleteVO((List<ISuperVO>) originGVOs);
			...
			AceQctestlistDeleteBP deleteBP = new AceQctestlistDeleteBP();
	...
	// 修改
	public AggTestlistHVO[] pubupdateBills(AggTestlistHVO[] vos)
			throws BusinessException {
		try {
			BillTransferTool<AggTestlistHVO> transTool = new BillTransferTool<AggTestlistHVO>((AggTestlistHVO[]) vos); 
			AggTestlistHVO[] fullBills = transTool.getClientFullInfoBill();
			AggTestlistHVO[] originBills = transTool.getOriginBills(); 
			
			// 孙VO的修改
			// nc.impl.pubapp.pattern.data.vo.template.UpdateBPTemplate

			AggTestlistHVO[] aggvos = (AggTestlistHVO[]) vos;
			...
			for (String tableCode : tableCodes) {
				...
				for (SuperVO childVO : originChildrens) { 
					// 将当前页签下的当前子的所有孙都查询出来了,并赋值给originBills中的孙。
					if (tableCode.equals("pk_testlistcomp")) {
						String originChildPK = ((TestlistcompBVO) childVO).getPrimaryKey();
						Collection originRVOs = query.queryBillOfVOByCond(
								TestlistentryEVO.class, "pk_testlistcomp = '"+ originChildPK + "' and dr=0", false);
						if(originRVOs != null && originRVOs.size()!=0){
							TestlistentryEVO[] originGrandvos = (TestlistentryEVO[]) originRVOs.toArray(new TestlistentryEVO[originRVOs.size()]);
							((TestlistcompBVO) childVO).setPk_testlistentry(originGrandvos);
				...
				for (SuperVO childVO : currentChildrens) {
					ISuperVO[] currentGrandvos = (TestlistentryEVO[]) ((TestlistcompBVO) childVO).getPk_testlistentry();
					for (int i = 0; currentGrandvos != null && i < currentGrandvos.length; i++) {
						((TestlistentryEVO) currentGrandvos[i]).setPk_testlistcomp((childVO.getPrimaryKey()));
					}
			...
			AceQctestlistUpdateBP bp = new AceQctestlistUpdateBP();
			AggTestlistHVO[] retBills = bp.update(fullBills, originBills);
...
private List<ISuperVO> get(IVOMeta voMeta,
	...
	public AggTestlistHVO[] pubquerybills(IQueryScheme queryScheme)
			throws BusinessException {
		AggTestlistHVO[] bills = null;
		try {
			this.preQuery(queryScheme);
			BillLazyQuery<AggTestlistHVO> query = new BillLazyQuery<AggTestlistHVO>(
					AggTestlistHVO.class);
